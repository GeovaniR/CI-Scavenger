...
def count_jobs_runs(i, runs, n_jobs_list, username, token, full_repo_path): # Função que conta o número de jobs em cada run
    ...
    if (conclusion != "action_required" and conclusion != "Skipped"): # Testa se o acesso as informações da run não é fechado
...
def get_pipeline_structure(username, token, full_repo_path, runs):
    i = 0
    conclusion = runs[i].get("conclusion") # Recupera estado de conclusão da run
    while (conclusion != "success" and conclusion != "failure"): # Testa se o acesso as informações da run não é fechado
        i += 1
    ...
    for i in range (0, n_jobs):
        name_job = jobs[i].get("name")
        jobs_structure = jobs_structure + name_job + " / "
    jobs_structure = jobs_structure[0:len(jobs_structure)-3]
