...
def investigate_workflow_keywords(i, n, workflows, request_path, username, token, full_repo_path):
    ...
    if (n_runs < n): # Ignora o caso de Pipelines que não são utilizados, mas estão no actions do repo   
        for i in range(0, n_runs): # Faz o loop das funções para o máximo de runs dísponível, mesmo que menor que n
            keywords_dict = verify_words_run_to_run(i, runs, keywords_dict, full_repo_path, username, token)                   
    else:   
        for i in range(0, n):
            keywords_dict = verify_words_run_to_run(i, runs, keywords_dict, full_repo_path, username, token)
...
def verify_words_run_to_run(i, runs, keywords_dict, full_repo_path, username, token): # i percore o número de runs
