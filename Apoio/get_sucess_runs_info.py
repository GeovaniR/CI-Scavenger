...
def calculate_run_sucess_execution_time(runs_sucess, i, runs_time_list):
    ...
    run_start_date = datetime(year = int(run_time_start[0:4]), month = int(run_time_start[5:7]), day = int(run_time_start[8:10]), hour = int(run_time_start[11:13]), minute = int(run_time_start[14:16]), second = int(run_time_start[17:19])) # Formata a saída
    run_finish_date = datetime(year = int(rum_time_finish[0:4]), month = int(rum_time_finish[5:7]), day = int(rum_time_finish[8:10]), hour = int(rum_time_finish[11:13]), minute = int(rum_time_finish[14:16]), second = int(rum_time_finish[17:19])) # Formata saída
...
def count_runs_sucess(i, runs, sucess, private):
    ...
    elif (conclusion == "action_required"): # Verifica se a run tem acesso fechado
